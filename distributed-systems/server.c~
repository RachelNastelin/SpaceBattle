...
// find a valid parent for the client, choosing randomly from the end of the
// list (since the list is in reverse order, this ensures that the client
// always connects to a parent older than it, to avoid loops)
void find_parent (int clients_remaining, server_rsp_t * response,
                  client_list_t * current) {
  srand((int)time(NULL));
  if (current == NULL) {
    response->parent_port = -1;
  } else {
    int rand_parent = (rand() % clients_remaining);
    for (int i=0; i<rand_parent; i++) {
      current = current->next;
    }
    strncpy(response->parent_ip, current->ip, INET_ADDRSTRLEN);
    response->parent_port = current->port_num;
    printf("Client given parent %d.\n", current->clientID);
  }
}
